...
sys.path.append("..")
from game.snake_game import Point, Direction
...
class Agent:
    ...
    def get_state(self,game,offset_check = 10):
        head = game.snake[0]
        point_l = Point(head.x - offset_check, head.y)
        point_r = Point(head.x + offset_check, head.y)
        point_u = Point(head.x, head.y - offset_check)
        point_d = Point(head.x, head.y + offset_check)
